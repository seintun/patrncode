      ...
      <Card className="max-w-md border-[var(--color-error)]/30 bg-[var(--color-bg-elevated)]/90 p-8 shadow-2xl animate-in zoom-in-95 duration-300">
        <div className="mb-6 flex justify-center">
          <div 
            ...
            style={{ animation: 'glow 2s ease-in-out infinite', '--sophia-glow-color': 'rgba(239, 68, 68, 0.3)' } as React.CSSProperties}
          >
        ...
        <p className="mb-8 text-center text-sm text-[var(--color-text-secondary)] leading-relaxed">
          Time is up for your <span style={{ color: sophiaConfig.colors.text }} className="font-semibold">{mode.replace('_', ' ')}</span> session on <span className="font-medium text-[var(--color-text-primary)]">{problemTitle || 'this problem'}</span>.
        </p>
        ...
        <div className="flex flex-col gap-3">
          ...
          <Button variant="secondary" onClick={onReturnToPractice} className="w-full bg-transparent border-[var(--color-border)] text-[var(--color-text-secondary)] hover:bg-[var(--color-bg-secondary)]" size="lg">
            Return to Practice
          </Button>
        </div>
      </Card>
